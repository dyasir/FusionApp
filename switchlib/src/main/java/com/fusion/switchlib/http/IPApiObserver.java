...
public abstract class IPApiObserver<T> implements Observer<IPApiResponse<T>> {
    ...
    public void onError(Throwable throwable) {
        ...
        if (throwable instanceof SocketTimeoutException  //网络超时,网络连接异常
            ...
            error = TextUtils.isEmpty(((DataResultException) throwable).getMsg()) ?
                    "Bad" : ((DataResultException) throwable).getMsg();
            ...
            if (((HttpException) throwable).code() == 401) {
                ...
                onFail(NET_ERROR, throwable.getMessage());
            } else {
                error = throwable.getMessage();
...
//            onFail(DATA_ERROR, error);
    ...
    public void onFinish(IPApiResponse<T> response) {
    }
