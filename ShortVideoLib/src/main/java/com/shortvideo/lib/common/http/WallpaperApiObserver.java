...
public abstract class WallpaperApiObserver<T> implements Observer<WallpaperApiResponse<T>> {
    ...
    @Override
    public void onNext(@NonNull WallpaperApiResponse<T> response) {
        //在这边对 基础数据 进行统一处理  初步解析：
        if (response.getCode() == 0) {
            onSuccess(response.getRes(), response.getMsg() == null ? "" : response.getMsg());
        } else {
    ...
    public void onError(Throwable throwable) {
        ...
        if (throwable instanceof SocketTimeoutException  //网络超时,网络连接异常
            ...
            Logger.e(((TokenBreakException) throwable).getCode() +
                    ", msg: " + ((TokenBreakException) throwable).getMsg());
...
//            onFail(DATA_ERROR, error);
    ...
    public void onFinish(WallpaperApiResponse<T> response) {
    }
